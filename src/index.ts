...
export default class Delcom {
  ...
  async init() {
    ...
    socket.on(
      ...
      async (fileNames: string[], callback: callbackWithErr) => {
        try {
          ...
          this._config.job = {
            dir: await fsp.mkdtemp(`${this._config.delcomTempDir}${path.sep}job`),
            writeStreams: {},
          };
          console.log(`New job files will be stored at ${this._config.job.dir}`);
    ...
    socket.on(
      ...
      (data: {name: string, chunk: string | Buffer}, callback: callbackWithErr) => {
        try {
          if (!this._config.job) {
    ...
    return new Promise<void>((res) => socket.on(
      'connect',
      async () => {
  ...
  private buildContainer() {
    return new Promise<void>((res, rej) => {
      ...
      const build = spawn(
        'docker',
        ['build', `-t${dockerName}`, '--progress=plain', dir],
      );
  ...
  private async createJob(
    ...
    try {
      ...
      this._config.res = {
        dir: outDir ? outDir : await fsp.mkdtemp(`${this._config.delcomTempDir}${path.sep}res`),
        writeStreams: {},
        finishPromise: {},
      };
      ...
      const fileNames = await Promise.all(filePaths.map(async (filePath) => {
      ...
      const ack = await this._config.socket?.emitWithAck('new_job_ack', workerID, fileNames);
      if (ack) {
        throw ack;
      }
    } catch (err) {
